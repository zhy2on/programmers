...
int calc_len(int n);
void convert_char(char* tmp, int n, int len);
int main(){
	char* answer, **tmp;
	int numbers_len, i, j, total_len = 0, len, *numbers;
			
	//배열 할당
	scanf("%d", &numbers_len);
	numbers = (int*)malloc(numbers_len * sizeof(int));
	tmp = (char**)malloc(numbers_len * sizeof(char*));

	//1. 각 원소 입력 받고 포인터 배열에 문자열로 저장
	for(i = 0; i < numbers_len; i++){
		scanf("%d", &numbers[i]);
		len = calc_len(numbers[i]);
		tmp[i] = (char*)malloc((len+1) * sizeof(char)); //배열 할당
		convert_char(tmp[i], numbers[i], len); //문자열로 저장
		total_len += len;
	}
...
int calc_len(int n){ //n 길이 구하는 calc_len 구현
	int len = 0;
	if(n == 0) return 1;
	while(n != 0){ n /= 10;	len++; }
	return len;
